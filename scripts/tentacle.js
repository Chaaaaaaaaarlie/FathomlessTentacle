...
function dbg(msg, data) {
  ...
  ChatMessage.create({ speaker: ChatMessage.getSpeaker(), content: `<small>[Debug] ${foundry.utils.escapeHTML(msg)}</small>`, whisper: [game.user.id] }).catch(()=>{});
}
...
async function hasLOS(casterToken, point) {
  // robuste Sichtprüfung in v13
  try { return await canvas.effects.visibility.testVisibility(point, { object: casterToken }); }
  catch { // Fallback
...
function nearestFree(center, casterCenter, maxFeet, requireSight, casterToken) {
  ...
  for (const [ox,oy] of spiral(maxCells)) {
    ...
    if (requireSight && !canvas.effects.visibility.testVisibility(c, { object: casterToken })) continue;
...
Hooks.once("ready", () => {
  // Hotkey Alt+T (für alle, nicht restricted)
  ...
  Hooks.on("chatMessage", (log, msg, chatData) => {
      ...
      if (msg.op === "attack") {
        ...
        if (!item) {
          ...
          const [created] = await tok.actor.createEmbeddedDocuments("Item", [{
            ...
            system: { level:0, activation:{type:"bonus",cost:1}, target:{value:1,type:"creature"}, range:{value:10,units:"ft"},
              actionType:"msak", damage:{parts:[["1d8","cold"]]}, scaling:{mode:"none"}, preparation:{mode:"innate",prepared:true} }
          }]);
          item = created; ephemeral=true;
        }
        const hasMidi = game.modules.get("midi-qol")?.active && typeof MidiQOL?.completeItemRoll === "function";
        if (hasMidi) {
          await MidiQOL.completeItemRoll(item, { configureDialog:true, targetUuids:[targetUuid], workflowOptions:{ autoRollDamage:"onHit" } });
...
async function summonTentacle() {
  ...
  // --- Zielpunkt wählen: einfacher Canvas-Klick mit Live-Overlay ---
  ui.notifications.info(`Klicke einen Punkt ≤ ${maxDist} ft${requireSight?" (Sichtlinie)":""
  }.`);

  // Vorschau-Overlay
  const overlay = new PIXI.Graphics(); overlay.zIndex = 10000; canvas.stage.addChild(overlay);

  const moveHandler = (ev) => {
    const p = ev.data.getLocalPosition(canvas.app.stage);
    const c = snapCenter(p);
    overlay.clear();
    overlay.lineStyle(2, 0x0088ff, 0.9).beginFill(0x00aaff, 0.25);
    const gs = canvas.grid.size; overlay.drawRect(c.x - gs/2, c.y - gs/2, gs, gs); overlay.endFill();
  };
  ...
  try { canvas.stage.removeChild(overlay); overlay.destroy(true); } catch {}
  ...
  let spawnCenter = isOccupied(click) ? (nearestFree(click, casterToken.center, maxDist, requireSight, casterToken) ?? click) : click;

  // Kurze Vorschau
  const gs = canvas.grid.size;
  const preview = canvas.grid.addHighlightLayer("ft-preview");
  preview.clear();
  canvas.grid.highlightPosition("ft-preview", { x: spawnCenter.x - gs/2, y: spawnCenter.y - gs/2, color: 0x00aaff, alpha: 0.5, border: 0x004466 });
  setTimeout(()=> preview.clear(), 900);
  ...
  const targetId = await Dialog.prompt({ title:"Ziel wählen", content:`<label>Ziel</label><select id="tgt">${opts}</select>`, label:"Weiter", callback: html => html.find("#tgt").val() });
  ...
  // Vorherige Tentacles dieses Casters entfernen
